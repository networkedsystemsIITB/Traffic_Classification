#---------------------------------------------KNN approach using 8 features--------------------------------------
'''This scripts  does real-time classification of the flows and prioritization/de-prioritization.
...
def main(argv):
	dev = "eth1"										# interface on which packet will be captured

	cap = pcapy.open_live(dev , 100 , 1 , 0) 			# Here, open_live() captures packet
...
def parse_packet(packet) :
	...
	if eth_protocol == 8 :								# checks if it is an IP packet
		...
		if protocol == 6 :								# protocol 6 referes to TCP protocol
			...
			if (len(main_packet_size[key]) == 200):									# This is to just print on screen that 200 packets have arrived in this flow
				print key, "200 packets arrived so far......."	
			if (len(main_packet_size[key]) == 750):									# This is to just print on screen that 750 packets have arrived in this flow
			...
			if (len(main_packet_size[key]) == int(minPacket)):			# flow has reached Threshold point, Now call the function to calculate features and then classify the flow
				if("172.16.1" not in str(s_addr)):  					# ignore ACK packets going from 172.16.1.XX ip address, as we are interested in 				
...
def calculate(key,proto) :
	...
	testrow = str(avg_pkt_size)+","+str(std_dev_pkt_size)+","+str(total_bytes)+","+str(round(float(avg_iat),2))+","+str(std_dev_IAT)+","+str(flow_duration)+","+str(push_flags[key])+","+str(avg_win_size)+","+"download" #Here 'download' is just a dummy class, as it is needed be be there in test arff  file


	#Remove the test-data line and blank lines....using sed commands
	
	subprocess.call("sed -i '/^[0-9]/d' test-data.arff", shell=True)		# remove existing test rows
	subprocess.call("sed -i '/^$/d' test-data.arff", shell=True)			# Remove blank lines

																			
	with open("test-data.arff", "a") as myfile:								#open the test data file, Append this new flow statistics 
	    myfile.write("\n"+testrow)
	    myfile.close(); 		
	
	
	
	#now invoke WEKA with the existing knn model, with this new test data file, to get the predicted TAG for it.
	#if tag is multimedia ,then  prioritize this flow by executing iptables command

	subprocess.call("bash call-weka-and-find-tag.sh", shell=True) 			# This script invokes weka and gets the tag i.e. multimedia, download
	
	# The above script puts the tag in the 'tag.txt',  open this file and fetch it
