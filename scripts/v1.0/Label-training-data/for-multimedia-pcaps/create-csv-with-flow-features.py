...
for key in main_inter_arrival_time.keys():
	...
	if length > 0:
        ...
        if(len(main_packet_size[key]) >= min_number_of_packets ):            		# we consider a flow only if at least 'min_number_of_packets' in that flow
        	string[key]=string[key]+","+str(avg_iat_in_ms)+","+str(std_dev_IAT)+","\
        	+str(flow_duration)+","+str(push_flag_count[key])+","+str(window_size[key]/len(main_packet_size[key]))+","+"Multimedia" 
        	print string[key]
