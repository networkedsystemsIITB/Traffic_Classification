...
# Now this txt file is given to another script 'create-csv-with-flow-features-with-No-GET.py' , which will convert this packet level information to flow level information
# That script will generate a csv file which will contain multimedia flows along with 4-tuple information and feature values
...
#Now run python script to generate csv file, where Each row will represents a multimedia flow.
for f in *.txt
do
	python create-csv-with-flow-features-with-No-GET.py $f 1000 > $f.csv   # 1st param:txt file ; 2nd param : threshold points (no. of packets after which classification need to be done)
